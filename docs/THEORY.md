...
Now, we must limit the friction. Coulomb's law of friction tells us that the force of friction cannot exceed the objects' normal force multiplied by the frictional coefficient $\mu$, which is different for every material. Thus, we must clamp this value between $-J\mu$ and $J\mu$, where $J$ is the impulse we applied across the normal vector during our impulse resolution.

However, this does not stop rolling friction, when a ball is rolling along a surface, as the friction applied would resolve to zero (as the tangent force being applied would be near zero). So, we must add rolling friction as well. This is similar to regular friction and also has its own coefficient, except that it is applied specifically to counteract the object's current angular velocity by adding a rotational impulse. This can be calculated in a very simple way. We want an angular impulse to be applied such that the relative difference in the objects' angular velocities is zero, so we want $\omega\_{b2} - \omega\_{a2} = 0$ if $\omega\_{a2}$ and $\omega\_{b2}$ are the resulting angular velocities of $a$ and $b$. Now, we already saw that change in angular velocity is equal to the change in angular momentum (which is like angular impulse) divided by the object's moment of inertia. So, if $L$ is our angular impulse, then $\omega\_{a2} = \omega\_{a1} - \frac{L}{I\_a}$ and $\omega\_{b2} = \omega\_{b1} + \frac{L}{I\_b}$ if $\omega\_{a1}$ and $\omega\_{b1}$ are the initial angular velocities of $a$ and $b$ (we swapped the minus and plus signs because the impulse is applied in opposite directions). If we substitute these values in our original equation, we get that $\omega\_{b1} + \frac{L}{I\_b} - \omega\_{a1} + \frac{L}{I_a} = 0 \implies \omega\_{diff} + L(\frac{1}{I_a} + \frac{1}{I_b}) = 0$, where $\omega\_{diff}$ is the difference in initial angular velocity between $a$ and $b$. Solving for $L$, we obtain that

$$L = \frac{-\omega\_{diff}}{\frac{1}{I_a} + \frac{1}{I_b}}$$,

which would be the angular impulse that must be applied to each object to make their difference in angular velocity be zero. However, based on the Coulomb's law of friction, the force of rolling friction also can't exceed the normal force applied multiplied by the rolling friction coefficient $\mu\_{roll}$, so we must clamp the value here as well.
